...
function parseYmdLocal(ymd: string): Date | null {
  ...
  // Treat date-only values as UTC calendar dates so they do not drift by timezone.
...
expensesRouter.get("/", authRequired, async (req: AuthedRequest, res) => {
  ...
  if (from || to) {
    ...
    if (from) {
      ...
      if (!Number.isNaN(d.getTime())) (where.date as Prisma.DateTimeFilter).gte = d;
    }
    if (to) {
...
expensesRouter.post("/", authRequired, async (req: AuthedRequest, res) => {
  ...
  const created = await prisma.expense.create({
    data: {
      ...
      date: parseYmdLocal(parsed.data.date) ?? new Date(parsed.data.date),
...
expensesRouter.put("/:id", authRequired, async (req: AuthedRequest, res) => {
  ...
  const updated = await prisma.expense.update({
    ...
    data: {
      ...
      date: parsed.data.date ? (parseYmdLocal(parsed.data.date) ?? new Date(parsed.data.date)) : undefined,
      note: parsed.data.note,
    },
  });
